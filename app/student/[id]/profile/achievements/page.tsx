    ...
    <>
      ...
      <div style={{ minHeight: '100vh', background: BG, fontFamily: 'Nunito, sans-serif', paddingBottom: 80 }}>
        ...
        <div style={{ background: `linear-gradient(135deg, ${T} 0%, #9CE0E0 100%)`, position: 'relative', overflow: 'hidden' }}>
          <div style={{ position: 'absolute', top: -60, right: 100, width: 200, height: 200, borderRadius: '50%', background: 'rgba(255,255,255,0.08)' }} />

          <div style={{ maxWidth: 1080, margin: '0 auto', padding: '22px clamp(20px,4vw,48px) 34px', position: 'relative', zIndex: 1 }}>
            {/* Back + title */}
            <div style={{ display: 'flex', alignItems: 'center', gap: 16, marginBottom: 26 }}>
              <button onClick={() => router.back()}
                style={{ width: 44, height: 44, borderRadius: 14, background: 'rgba(255,255,255,0.2)', border: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
              ...
              <div style={{ color: 'white', fontWeight: 900, fontSize: 26 }}>Шагнал</div>
            </div>

            {/* Progress summary */}
            <div style={{ maxWidth: 520 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', marginBottom: 9 }}>
                <span style={{ color: 'white', fontWeight: 800, fontSize: 16 }}>{earnedCount} / {total} шагнал авсан</span>
                <span style={{ color: 'rgba(255,255,255,0.9)', fontWeight: 800, fontSize: 15 }}>{pct}%</span>
              </div>
              <div style={{ height: 10, background: 'rgba(255,255,255,0.22)', borderRadius: 5, overflow: 'hidden' }}>
                <div style={{ width: `${pct}%`, height: '100%', background: '#FFF3D6', borderRadius: 5, transition: 'width 0.5s' }} />
              </div>
            </div>
          </div>
        </div>
        ...
        <div style={{ maxWidth: 1080, margin: '0 auto', padding: '40px clamp(20px,4vw,48px) 0' }}>
          <div style={{
            background: 'white', borderRadius: 24, border: `1.5px solid ${BR}`,
            padding: '44px 32px', boxShadow: '0 1px 4px rgba(0,0,0,0.03)',
            display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(140px, 1fr))',
            gap: '40px 16px', justifyItems: 'center',
          }}>
            {achievements.map(a => {
              const isEarned = earnedIds.includes(a.id)
              return <AchTile key={a.id} ach={a} earned={isEarned} onClick={() => setSheet(a)} />
            })}
          </div>
          <div style={{ textAlign: 'center', fontSize: 13, color: S3, fontWeight: 600, marginTop: 20 }}>
            Шагнал дээр дарж дэлгэрэнгүйг хараарай
          </div>
        </div>
      </div>
      ...
      <AchModal
        ach={sheet}
        earned={sheet ? earnedIds.includes(sheet.id) : false}
